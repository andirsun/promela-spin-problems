...
short src_ln1 [] = {
	  0,  79,  81,  83,  85,  86,  88,  90, 
	 92,  93,  85,  96,  83,  97,  81,  99, 
	 99, 101, 103, 107, 109, 111, 113, 115, 
	105, 117, 117, 103, 119, 101, 120, 120, 
	120,  78, 122,  78, 122, 123,   0, };
S_F_MAP src_file1 [] = {
	{ "-", 0, 0 },
	{ "client-server.pml", 1, 37 },
	{ "-", 38, 39 }
};
uchar reached1 [] = {
	  0,   1,   1,   1,   1,   0,   0,   0, 
...
short src_ln0 [] = {
	  0,  42,  42,  44,  46,  50,  51,  52, 
	 53,  55,  55,  56,  53,  58,  48,  60, 
	 60,  60,  46,  63,  63,  63,  40,  66, 
	 40,  66,  67,  68,   0, };
S_F_MAP src_file0 [] = {
	{ "-", 0, 0 },
	{ "client-server.pml", 1, 27 },
	{ "-", 28, 29 }
};
uchar reached0 [] = {
	  0,   1,   1,   1,   1,   1,   0,   0, 
...
{	int j, h = now._nr_pr;
	...
	case 9:	/* claim select */
		...
		if (whichclaim == -1 && claimname == NULL)
			printf("0: Claim c1 (2), from state 3\n");
#endif
		if (whichclaim != -1)
	...
	case 1:	/* Receptor */
		...
		/* params: */
		/* locals: */
		((P1 *)pptr(h))->bitrecibido = 0;
		((P1 *)pptr(h))->bitanterior = 1;
		((P1 *)pptr(h))->x = 0;
#ifdef VAR_RANGES
		logval("Receptor:bitrecibido", ((P1 *)pptr(h))->bitrecibido);
		logval("Receptor:bitanterior", ((P1 *)pptr(h))->bitanterior);
		logval("Receptor:x", ((P1 *)pptr(h))->x);
#endif
#ifdef HAS_CODE
		locinit1(h);
#endif
	...
	case 0:	/* Emisor */
		...
		/* params: */
		/* locals: */
		((P0 *)pptr(h))->bitenvio = 0;
		((P0 *)pptr(h))->bitrecibido = 0;
#ifdef VAR_RANGES
		logval("Emisor:bitenvio", ((P0 *)pptr(h))->bitenvio);
		logval("Emisor:bitrecibido", ((P0 *)pptr(h))->bitrecibido);
#endif
#ifdef HAS_CODE
		locinit0(h);
#endif
...
iniglobals(int calling_pid)
{
	...
	{	int l_in;
		...
		now.idUltimo = 0;
		now.cont = 0;
		now.conf = addqueue(calling_pid, 1, 0);
		now.env = addqueue(calling_pid, 2, 0);
		now.file[0] = 3;
		now.file[1] = 5;
		now.file[2] = 1;
		now.file[3] = 6;
		...
		now.flag = 0;
#ifdef VAR_RANGES
		logval("fallado", now.fallado);
	{	int l_in;
		for (l_in = 0; l_in < 8; l_in++)
		{
			logval("data[l_in]", now.data[l_in]);
		}
	}
		logval("m", now.m);
		logval("idUltimo", now.idUltimo);
		logval("cont", now.cont);
	{	int l_in;
		for (l_in = 0; l_in < 4; l_in++)
		{
			logval("file[l_in]", now.file[l_in]);
		}
	}
...
{	int i = 0;
	...
	{	if (t->ty[i] == TIMEOUT_F) return (!(trpt->tau&1));
		...
		switch (t->qu[i]) {
		case 0: break;
		case 1: if (	(t->ty[i] == Q_FULL_F && ( q_full((int) ( now.conf) )))
			 || 	(t->ty[i] == Q_FULL_T && (!q_full((int) ( now.conf) )))
			 || 	(t->ty[i] == Q_EMPT_F && ( !q_len((int) ( now.conf) )))
			 || 	(t->ty[i] == Q_EMPT_T && (  q_len((int) ( now.conf) )))
			    ) return 0; break;
		case 2: if (	(t->ty[i] == Q_FULL_F && ( q_full((int) ( now.env) )))
			 || 	(t->ty[i] == Q_FULL_T && (!q_full((int) ( now.env) )))
			 || 	(t->ty[i] == Q_EMPT_F && ( !q_len((int) ( now.env) )))
			 || 	(t->ty[i] == Q_EMPT_T && (  q_len((int) ( now.env) )))
			    ) return 0; break;
...
{	/* int i; */
	...
	{	int l_in;
		for (l_in = 0; l_in < 8; l_in++)
		{
			printf("	byte   data[%d]:	%d\n", l_in, now.data[l_in]);
		}
	}
	printf("	mtype  MENS:	2\n");
	printf("	mtype  CNF:	1\n");
	printf("	chan conf (=%d):	len %d:\t", now.conf, q_len(now.conf));
	c_chandump(now.conf);
	printf("	chan env (=%d):	len %d:\t", now.env, q_len(now.env));
	c_chandump(now.env);
	{	int l_in;
		for (l_in = 0; l_in < 4; l_in++)
		{
	...
	printf("	bit    fallado:	%d\n", now.fallado);
	printf("	int    elems:	%d\n", now.elems);
	printf("	int    flag:	%d\n", now.flag);
	printf("	byte   m:	%d\n", now.m);
	printf("	byte   idUltimo:	%d\n", now.idUltimo);
	printf("	byte   cont:	%d\n", now.cont);
}
void
...
{	/* int i; */
	...
	case 2:
		...
		break;
	case 1:
		printf("local vars proc %d (Receptor):\n", pid);
	printf("	bit    bitrecibido:	%d\n", ((P1 *)pptr(pid))->bitrecibido);
	printf("	bit    bitanterior:	%d\n", ((P1 *)pptr(pid))->bitanterior);
	printf("	byte   x:	%d\n", ((P1 *)pptr(pid))->x);
		break;
	case 0:
		printf("local vars proc %d (Emisor):\n", pid);
	printf("	bit    bitenvio:	%d\n", ((P0 *)pptr(pid))->bitenvio);
	printf("	bit    bitrecibido:	%d\n", ((P0 *)pptr(pid))->bitrecibido);
		break;
	}
}
void
...
{
	switch (x) {
	case 1: Printf("CNF"); break;
	case 2: Printf("MENS"); break;
	default: Printf("%d", x);
	}
}
void
